...
test("Chlum, Nesměň and Besednice use in-repo production art without changing Slávie assets", () => {
...
test("Besednice production plate is one complete full-resolution JPEG", () => {
  const data = readFileSync(new URL("../assets/besednice/besednice-v7-plate-prod.jpg", import.meta.url));
  assert.ok(data.length > 600_000, "Besednice terrain should contain substantial image data");
  assert.ok(data.length < 1_200_000, "Besednice terrain should stay within its production budget");
  assert.deepEqual([...data.subarray(0, 2)], [0xff, 0xd8], "JPEG must start with SOI");
  assert.deepEqual([...data.subarray(-2)], [0xff, 0xd9], "JPEG must end with EOI");
...
test("production characters and the Chlum foreground are complete transparent PNG assets", () => {
  const assets = [
    ...
    ["../assets/actors/pitkeeper-v7-prod.png", 720, 1040, 450_000, 700_000],
